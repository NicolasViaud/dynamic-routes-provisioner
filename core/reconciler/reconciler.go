...
func (r *Reconciler) Reconcile(ctx context.Context) error {
	...
	r.logger.Info("reconciliation diff computed",
	...
	if len(toRemove) > 0 {
		if err := r.provisioner.BatchDeprovision(ctx, toRemove); err != nil {
			r.logger.Error("batch deprovision failed", "error", err)
		}
	}
	...
	for _, route := range toAdd {
		...
		if err != nil {
			r.logger.Error("certificate issuance failed during reconciliation",
	...
	if len(toAdd) > 0 {
		...
		r.logger.Info("batch provision completed", "provisioned", len(results))
	}
