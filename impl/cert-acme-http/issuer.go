...
// Compile-time check that ACMEIssuer satisfies certificate.Issuer.
var _ certificate.Issuer = (*ACMEIssuer)(nil)

// ACMEIssuer obtains TLS certificates using the ACME HTTP-01 challenge.
// The developer provides a ChallengeSolver that controls how the challenge
// token is served.
...
// New creates an ACMEIssuer. If no account key is provided via options,
// a new ECDSA P-256 key is generated.
func New(solver ChallengeSolver, opts ...Option) (*ACMEIssuer, error) {
	cfg := defaultConfig()
	for _, o := range opts {
		o(&cfg)
	}
...
// Name returns "acme-http-01".
func (a *ACMEIssuer) Name() string { return "acme-http-01" }

// Issue obtains a certificate for the host in req using the ACME HTTP-01 flow.
func (a *ACMEIssuer) Issue(ctx context.Context, req core.RouteRequest) (*core.Certificate, error) {
...
func (a *ACMEIssuer) Revoke(ctx context.Context, cert core.Certificate) error {
...
// fulfillAuthorization finds the HTTP-01 challenge in the given authorization
// and completes it using the ChallengeSolver.
func (a *ACMEIssuer) fulfillAuthorization(ctx context.Context, domain, authzURL string) error {
	...
	if chal == nil {
		return fmt.Errorf("no http-01 challenge found for %s", domain)
	}
	...
	// Present the challenge via the solver.
	if err := a.solver.Present(ctx, domain, chal.Token, keyAuth); err != nil {
		return fmt.Errorf("present challenge: %w", err)
	}
	defer a.solver.Cleanup(ctx, domain, chal.Token)
